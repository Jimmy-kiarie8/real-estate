<template>
    <MainLayout title="Order Management">
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <v-data-table :headers="headers" :items="projects.data" :sort-by="[{ key: 'name', order: 'asc' }]" class="elevation-2" :search="search">
                <template v-slot:top>
                <v-toolbar flat>
...
export default {
    ...
    computed: {
      pageCount () {
        return Math.ceil(this.projects.data.length / this.itemsPerPage)
      },
    },
  }
</script>
